defmodule Silicon.Ed25519Test do
  ...
  test "key_pair" do
    ...
    |> Enum.each(fn _ ->
      ...
      assert secret_key_to_public_key(secret_key) == {:ok, pubkey}
    end)
  end
  ...
  test "secret_to_public_key/1" do
    ...
    |> Enum.each(fn [sk, pk, _msg, _sig_msg] ->
      ...
      assert secret_to_public_key(secret) == {:ok, pk}
    end)
  end
  ...
  test "secret_key_to_public_key/1" do
    @vectors
    |> Enum.each(fn [sk, pk, _msg, _sig_msg] ->
      assert secret_key_to_public_key(sk) == {:ok, pk}
    end)
  end
  ...
  test "secret_key_to_secret/1" do
    ...
    |> Enum.each(fn [sk, _pk, _msg, _sig_msg] ->
      <<secret::bytes-32, _::binary>> = sk
      assert secret_key_to_secret(sk) == {:ok, secret}
    end)
  end
  ...
  test "sign/2 and verify/3" do
    ...
    |> Enum.each(fn [sk, pk, msg, sig_msg] ->
      ...
      assert sign(msg, sk) == {:ok, sig}
      assert verify(sig, msg, pk) == :ok
    end)
  end
end
